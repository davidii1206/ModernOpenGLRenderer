...
Replaces the old flat float32 atlas dumps with three self-describing, GPU-traversable
sparse mip chains (~30 MB total vs ~3 GB of float texels).
...
```
atlas_depth.bin      RG8  per node: (dmin, dmax)   quantised to [gdmin, gdmax]
...
| 4      | u32  | version (1) |
| 8      | u32  | channels (1 = R8, 2 = RG8) |
| 12     | u32  | bytes per channel (1) |
| 16     | u32  | num_levels |
...
| data_size | u64 — `w*h*channels` bytes |
...
* **Value texture** — `data_off..+data_size`, `R8` or `RG8`, byte per channel:
  `byte = 1 + round(clamp((v - qmin[c]) / (qmax[c] - qmin[c]), 0, 1) * 254)`,
  decoded as `v = qmin[c] + (qmax[c] - qmin[c]) * (byte - 1) / 254`.
  Byte `0` is reserved for an **empty node** (no covered texels); every covered
  node stores `1..255`, so a valid value is never `0`/black and consumers can
  test `byte != 0` (or meta bit 31) before decoding.
...
value = data[L][idx]            # 1 or 2 bytes, decoded with qmin/qmax
```
...
| chain | channels | nodes (Sponza) | file size |
|-------|---------:|---------------:|----------:|
| depth | 2 | 1,815,700 | 10.39 MB |
| thickness | 1 | 1,815,700 | 8.66 MB |
| uv | 2 | 1,815,700 | 10.39 MB |

9 levels (L0 4,576 → L8 568,848 nodes). The three chains share one topology.
