...
void Pipeline::set(const char* name, int v) const        { MOSAIC_UNIFORM(prog_->uniform1i(loc, v)); }
void Pipeline::set(const char* name, unsigned v) const   { MOSAIC_UNIFORM(prog_->uniform1ui(loc, v)); }
void Pipeline::set(const char* name, float v) const      { MOSAIC_UNIFORM(prog_->uniform1f(loc, v)); }
void Pipeline::set(const char* name, const glm::vec2& v) const  { MOSAIC_UNIFORM(prog_->uniform2f(loc, v.x, v.y)); }
void Pipeline::set(const char* name, const glm::vec3& v) const  { MOSAIC_UNIFORM(prog_->uniform3f(loc, v.x, v.y, v.z)); }
void Pipeline::set(const char* name, const glm::vec4& v) const  { MOSAIC_UNIFORM(prog_->uniform4f(loc, v.x, v.y, v.z, v.w)); }
void Pipeline::set(const char* name, const glm::ivec2& v) const { MOSAIC_UNIFORM(prog_->uniform2iv(loc, glm::value_ptr(v))); }
void Pipeline::set(const char* name, const glm::ivec3& v) const { MOSAIC_UNIFORM(prog_->uniform3iv(loc, glm::value_ptr(v))); }
void Pipeline::set(const char* name, const glm::mat3& v) const  { MOSAIC_UNIFORM(prog_->uniform_matrix3fv(loc, glm::value_ptr(v))); }
void Pipeline::set(const char* name, const glm::mat4& v) const  { MOSAIC_UNIFORM(prog_->uniform_matrix4fv(loc, glm::value_ptr(v))); }

#undef MOSAIC_UNIFORM

// --- PrefixSum --------------------------------------------------------------

bool PrefixSum::init() {
    block_  = Pipeline::compute("shaders/scan_block.comp");
    blocks_ = Pipeline::compute("shaders/scan_blocks.comp");
    finish_ = Pipeline::compute("shaders/scan_finish.comp");
    block_sums_.data(nullptr, kScanBlocksMax * sizeof(uint32_t));
    return block_.valid() && blocks_.valid() && finish_.valid();
}

bool PrefixSum::poll() {
    bool changed = block_.poll();
    changed |= blocks_.poll();
    changed |= finish_.poll();
    return changed;
}

void PrefixSum::run(gl::Buffer& counts, gl::Buffer& starts, uint32_t n, uint32_t off) {
    if (n == 0) return;
    if (n > kMaxElements) {
        gllib::logf(gllib::LogLevel::error,
                    "PrefixSum::run: %u elements exceeds the single-workgroup limit of %u",
                    n, kMaxElements);
        return;
    }
    ...
    counts.bind_base(0);
    starts.bind_base(1);
    block_sums_.bind_base(2);
    block_.use();
    block_.set("u_n", n);
    block_.set("u_off", off);
    gl::dispatch_compute(nblocks, 1, 1);
    gl::memory_barrier(GL_SHADER_STORAGE_BARRIER_BIT);

    block_sums_.bind_base(0);
    blocks_.use();
    blocks_.set("u_nblocks", nblocks);
    gl::dispatch_compute(1, 1, 1);
    gl::memory_barrier(GL_SHADER_STORAGE_BARRIER_BIT);

    starts.bind_base(0);
    block_sums_.bind_base(1);
    finish_.use();
    finish_.set("u_n", n);
    finish_.set("u_off", off);
    gl::dispatch_compute(nblocks, 1, 1);
    gl::memory_barrier(GL_SHADER_STORAGE_BARRIER_BIT);
}
