...
layout(binding = 4) uniform sampler2D u_recon;      // (E, 1) from the gather
...
void main() {
    ...
    // The gather writes (E, 1), so this divide is a no-op on a reached pixel and
    // keeps an unreached one at zero rather than exploding on a ~0 alpha.
    ...
    switch (u_view_mode) {
        ...
        case 8: color = heat(acc.a); break;                     // gather fallback flag
